...
class EdgeAwareAttentionConv(MessagePassing):
    """Edge-aware attention message passing for cell interaction graphs."""
    ...
    def __init__(
        ...
        *,
        ...
        message_in_dim = hidden_dim * 2 + edge_dim
        self.message_mlp = MLP(message_in_dim, hidden_dim, hidden_dim, num_layers=2, dropout=dropout)
        self.attention_mlp = MLP(message_in_dim, hidden_dim, 1, num_layers=2, dropout=dropout)
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor) -> Tensor:
        ...
        if edge_attr.dim() != 2:
            raise ValueError(f"Expected edge_attr shape [E, Fe], got {tuple(edge_attr.shape)}")
    ...
    def message(
        ...
        return self.message_mlp(pair) * alpha.unsqueeze(-1)


class TemporalGRUCell(nn.Module):
    """Optional node-wise temporal state updater for future rollout support."""
