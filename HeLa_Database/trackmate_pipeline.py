...
@dataclass
class Config:
    neighbor_radii: tuple[float, ...] = (20.0, 40.0, 80.0, 160.0)
    neighbor_decays: tuple[float, ...] = (15.0, 30.0, 60.0, 120.0)
    k_nearest: int = 5
    n_sectors: int = 8
    ...
    def neighbor_radius(self):
        radii, _ = normalized_neighbor_scales(self)
        return _default_neighbor_radius(radii)

    @property
    def decay(self):
        radii, decays = normalized_neighbor_scales(self)
        default_radius = _default_neighbor_radius(radii)
        for radius, decay in zip(radii, decays):
            if math.isclose(radius, default_radius, rel_tol=0.0, abs_tol=1e-9):
                return decay
        return decays[0]


def parse_float_tuple(value, option_name):
...
def normalized_neighbor_scales(cfg: Config):
    radii = parse_float_tuple(cfg.neighbor_radii, "neighbor_radii")
    decays = parse_float_tuple(cfg.neighbor_decays, "neighbor_decays")
    if len(radii) != len(decays):
        raise ValueError("neighbor_radii and neighbor_decays must have same length.")
    if any(radius <= 0 for radius in radii):
        raise ValueError("neighbor_radii must be positive.")
    if any(decay <= 0 for decay in decays):
        raise ValueError("neighbor_decays must be positive.")
    ordered = sorted(zip(radii, decays), key=lambda item: item[0])
    return tuple(radius for radius, _ in ordered), tuple(decay for _, decay in ordered)


def neighbor_scale_label(radius):
    if math.isclose(radius, round(radius), rel_tol=0.0, abs_tol=1e-9):
        return str(int(round(radius)))
    return f"{radius:g}".replace("-", "m").replace(".", "p")


def _default_neighbor_radius(radii):
    for radius in radii:
        if math.isclose(radius, 40.0, rel_tol=0.0, abs_tol=1e-9):
            return radius
    return radii[min(1, len(radii) - 1)]
...
def _point_to_segment_distance(point, start, end):
    ...
    if denom <= 1e-12:
        return _safe_norm(point - start)
    t = float(np.dot(point - start, segment) / denom)
    t = min(1.0, max(0.0, t))
    projection = start + t * segment
    return _safe_norm(point - projection)


def compute_distance_to_convex_hull(points):
    points = np.asarray(points, dtype=np.float64)
    n_points = len(points)
    if n_points == 0:
    ...
    if len(unique_points) < 3:
        return np.zeros(n_points, dtype=np.float32), np.ones(n_points, dtype=bool)
    ...
    segment_start = hull_points
    segment_end = np.roll(hull_points, -1, axis=0)
    ...
    boundary_eps = max(1e-6, scale * 1e-6)
    return distances.astype(np.float32), distances <= boundary_eps


def _neighbor_feature_columns(radii, n_sectors):
    columns = ["distance_to_colony_edge", "is_boundary_cell"]
    for radius in radii:
        label = neighbor_scale_label(radius)
        columns.extend([
            f"n_neighbors_r{label}",
            f"density_r{label}",
            f"Fx_r{label}",
            f"Fy_r{label}",
            f"F_norm_r{label}",
            f"min_dist_r{label}",
            f"mean_dist_r{label}",
            f"free_space_r{label}",
            f"occupancy_area_r{label}",
            f"density_grad_x_r{label}",
            f"density_grad_y_r{label}",
        ])
        columns.extend(f"sector_r{label}_{sector}" for sector in range(n_sectors))

    for inner, outer in zip(radii[:-1], radii[1:]):
        inner_label = neighbor_scale_label(inner)
        outer_label = neighbor_scale_label(outer)
        columns.extend([
            f"ring_count_{inner_label}_{outer_label}",
            f"ring_density_{inner_label}_{outer_label}",
        ])
    return columns


def compute_multiscale_neighbor_features(frame_df, radii, decays, n_sectors):
    pts = frame_df[["x", "y"]].to_numpy(dtype=np.float64)
    n_points = len(frame_df)
    feature_columns = _neighbor_feature_columns(radii, n_sectors)
    if n_points == 0:
        return pd.DataFrame(columns=feature_columns, index=frame_df.index)

    features = pd.DataFrame(index=frame_df.index)
    edge_distance, boundary = compute_distance_to_convex_hull(pts)
    features["distance_to_colony_edge"] = edge_distance
    features["is_boundary_cell"] = boundary

    tree = cKDTree(pts)
    max_radius = max(radii)
    neighbor_lists = tree.query_ball_point(pts, max_radius)
    area_values = (
        pd.to_numeric(frame_df["AREA"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
        if "AREA" in frame_df
        else np.zeros(n_points, dtype=np.float64)
    )
    sector_width = 2.0 * math.pi / n_sectors if n_sectors > 0 else 2.0 * math.pi
    counts_by_radius: dict[float, np.ndarray] = {}

    for radius, decay in zip(radii, decays):
        label = neighbor_scale_label(radius)
        circle_area = math.pi * radius**2

        n_neighbors = np.zeros(n_points, dtype=np.int32)
        density = np.zeros(n_points, dtype=np.float64)
        fx = np.zeros(n_points, dtype=np.float64)
        fy = np.zeros(n_points, dtype=np.float64)
        min_dist = np.full(n_points, np.nan, dtype=np.float64)
        mean_dist = np.full(n_points, np.nan, dtype=np.float64)
        free_space = np.ones(n_points, dtype=np.float64)
        occupancy_area = np.zeros(n_points, dtype=np.float64)
        sectors = np.zeros((n_points, n_sectors), dtype=np.int32)

        for index, neighbors in enumerate(neighbor_lists):
            candidates = [candidate for candidate in neighbors if candidate != index]
            if not candidates:
                continue

            vectors = pts[candidates] - pts[index]
            distances = np.linalg.norm(vectors, axis=1)
            within = distances <= radius
            if not within.any():
                continue

            selected_candidates = np.asarray(candidates, dtype=np.int64)[within]
            selected_vectors = vectors[within]
            selected_distances = distances[within]
            positive = selected_distances > 1e-12

            count = int(len(selected_distances))
            n_neighbors[index] = count
            density[index] = count / circle_area
            min_dist[index] = float(np.min(selected_distances))
            mean_dist[index] = float(np.mean(selected_distances))
            free_space[index] = min(1.0, min_dist[index] / radius)
            occupancy_area[index] = float(np.nansum(area_values[selected_candidates]) / circle_area)

            if positive.any():
                unit = selected_vectors[positive] / selected_distances[positive, None]
                weights = np.exp(-selected_distances[positive] / decay)
                weighted = unit * weights[:, None]
                fx[index] = float(weighted[:, 0].sum())
                fy[index] = float(weighted[:, 1].sum())

                if n_sectors > 0:
                    angles = np.mod(np.arctan2(selected_vectors[positive, 1], selected_vectors[positive, 0]), 2.0 * math.pi)
                    sector_ids = np.floor(angles / sector_width).astype(int)
                    sector_ids = np.clip(sector_ids, 0, n_sectors - 1)
                    sectors[index] = np.bincount(sector_ids, minlength=n_sectors)

        counts_by_radius[radius] = n_neighbors
        features[f"n_neighbors_r{label}"] = n_neighbors
        features[f"density_r{label}"] = density
        features[f"Fx_r{label}"] = fx
        features[f"Fy_r{label}"] = fy
        features[f"F_norm_r{label}"] = np.hypot(fx, fy)
        features[f"min_dist_r{label}"] = min_dist
        features[f"mean_dist_r{label}"] = mean_dist
        features[f"free_space_r{label}"] = free_space
        features[f"occupancy_area_r{label}"] = occupancy_area
        features[f"density_grad_x_r{label}"] = fx / circle_area
        features[f"density_grad_y_r{label}"] = fy / circle_area
        for sector in range(n_sectors):
            features[f"sector_r{label}_{sector}"] = sectors[:, sector]

    for inner, outer in zip(radii[:-1], radii[1:]):
        inner_label = neighbor_scale_label(inner)
        outer_label = neighbor_scale_label(outer)
        ring_count = counts_by_radius[outer] - counts_by_radius[inner]
        ring_area = math.pi * (outer**2 - inner**2)
        features[f"ring_count_{inner_label}_{outer_label}"] = ring_count
        features[f"ring_density_{inner_label}_{outer_label}"] = ring_count / ring_area

    return features


def add_neighbors(df, cfg: Config):
    radii, decays = normalized_neighbor_scales(cfg)
    if cfg.n_sectors < 1:
        raise ValueError("n_sectors must be positive.")

    if df.empty:
        df = df.copy()
        for column in _neighbor_feature_columns(radii, cfg.n_sectors):
            df[column] = []
        default_label = neighbor_scale_label(_default_neighbor_radius(radii))
        df["n_neighbors"] = []
        df["density"] = []
        df["Fx"] = []
        df["Fy"] = []
    ...
    for frame, g in df.groupby("frame"):
        g = g.copy()
        feature_df = compute_multiscale_neighbor_features(g, radii, decays, cfg.n_sectors)
        g = pd.concat([g, feature_df], axis=1)

        out.append(g)

    result = pd.concat(out)
    default_label = neighbor_scale_label(_default_neighbor_radius(radii))
    result["n_neighbors"] = result[f"n_neighbors_r{default_label}"]
    result["density"] = result[f"density_r{default_label}"]
    result["Fx"] = result[f"Fx_r{default_label}"]
    result["Fy"] = result[f"Fy_r{default_label}"]
    result["F_norm"] = result[f"F_norm_r{default_label}"]
    return result
