...
def main() -> None:
    ...
    print("loaders", sorted(loaders.keys()), flush=True)
    rows = []
    best = None

    for pos_weight in POS_WEIGHTS:
        print(f"=== train bce_pos_weight={pos_weight} ===", flush=True)
        ...
        for epoch in range(CONFIG["epochs"]):
            train_metrics = train_epoch(
                ...
                loss_mode=CONFIG["loss_mode"], lambda_intermediate=CONFIG["lambda_intermediate"], lambda_hidden_l2=CONFIG["lambda_hidden_l2"],
                ...
                bce_pos_weight=pos_weight, lambda_dice=CONFIG["lambda_dice"], show_progress=False,
            )
            history.append({"epoch": epoch, **train_metrics})
            print(f"pos_weight={pos_weight} epoch={epoch} train_loss={train_metrics['loss']:.6f} final={train_metrics['final_loss']:.6f}", flush=True)

        model_dir = run_dir / f"pos_weight_{pos_weight:g}"
        model_dir.mkdir(parents=True, exist_ok=True)
        save_checkpoint(
            model_dir / "checkpoint_latest.pt", model=model, optimizer=optimizer, epoch=CONFIG["epochs"] - 1,
            config={"config": CONFIG, "bce_pos_weight": pos_weight, "normalizer": normalizer.state_dict()}, metrics=history[-1],
        )
        ...
        for threshold in THRESHOLDS:
            ...
            row = {
                "bce_pos_weight": pos_weight, "eval_threshold": threshold,
                **{f"val_rollout_{k}": v for k, v in metrics.items()},
                "train_loss": history[-1]["loss"], "train_final_loss": history[-1]["final_loss"], "train_intermediate_loss": history[-1]["intermediate_loss"],
            }
            rows.append(row)
            if best is None or row["val_rollout_iou"] > best["val_rollout_iou"]:
                best = row
            print(
                f"eval pos_weight={pos_weight} threshold={threshold:.2f} iou={row['val_rollout_iou']:.4f} dice={row['val_rollout_dice']:.4f} precision={row['val_rollout_precision']:.4f} recall={row['val_rollout_recall']:.4f}",
                flush=True,
            )
