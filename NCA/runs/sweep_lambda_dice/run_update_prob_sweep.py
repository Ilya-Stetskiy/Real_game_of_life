...
BASE = {
    ...
    "pattern": "*.npy",
    "run_dir": PROJECT_ROOT / "NCA" / "runs" / "sweep_update_prob",
    "split_mode": "by_file",
...
def main() -> None:
    ...
    rows = []
    best = None
    for update_prob in UPDATE_PROBS:
        print(f"=== train update_prob={update_prob:g} ===", flush=True)
        set_seed(BASE["seed"])
        model = NCA(
            state_channels=BASE["data_channels"] + BASE["hidden_channels"], model_width=BASE["model_width"], kernel_size=BASE["kernel_size"],
            update_prob=update_prob, use_alive_mask=BASE["use_alive_mask"], primary_channel=BASE["primary_channel"],
        ).to(device)
        ...
        for epoch in range(BASE["epochs"]):
            ...
            print(f"update_prob={update_prob:g} epoch={epoch} train_loss={train_metrics['loss']:.6f}", flush=True)
        ...
        stoch = stochastic_eval(
            model=model, loader=loaders["val_rollout"], device=device, num_rollouts=BASE["num_rollouts"], data_channels=BASE["data_channels"],
            hidden_channels=BASE["hidden_channels"], primary_channel=BASE["primary_channel"], supervised_loss=BASE["supervised_loss"], threshold=BASE["eval_threshold"], show_progress=False,
        )
        row = {
            "update_prob": update_prob,
            **{f"det_{k}": v for k, v in det.items()},
            **{f"stoch_{k}": v for k, v in stoch.items()},
        ...
        rows.append(row)
        if best is None or row["det_iou"] > best["det_iou"]:
            best = row
        model_dir = run_dir / f"update_prob_{update_prob:g}"
        model_dir.mkdir(parents=True, exist_ok=True)
        save_checkpoint(
            model_dir / "checkpoint_latest.pt", model=model, optimizer=optimizer, epoch=BASE["epochs"] - 1,
            config={"config": BASE, "update_prob": update_prob, "normalizer": normalizer.state_dict()}, metrics=row,
        )
        np.save(model_dir / "train_history.npy", np.asarray(history, dtype=object))
        print(f"eval update_prob={update_prob:g} det_iou={row['det_iou']:.4f} det_dice={row['det_dice']:.4f} stoch_iou={row['stoch_iou']:.4f} stoch_dice={row['stoch_dice']:.4f}", flush=True)
    fieldnames = sorted({key for row in rows for key in row.keys()})
    with (run_dir / "results.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)
    summary = {"rows": rows, "best_by_det_iou": best}
    (run_dir / "results.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print("=== best_by_det_iou ===", flush=True)
